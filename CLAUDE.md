...
**Push notifications (suivis en retard):** opt-in via a button in the bell dropdown (`enablePush` → `pushManager.subscribe` with the VAPID public key from `GET /api/push/vapid`, POSTed to `/api/push/subscribe`). The server ([server.cjs](server/server.cjs)) uses `web-push`; it needs env vars `VAPID_PUBLIC_KEY`, `VAPID_PRIVATE_KEY`, `VAPID_SUBJECT` — without them push is inert (`pushEnabled=false`). Subscriptions live in the `push_subs` collection. `computeOverdue()` mirrors the frontend bell (>30 days since last suivi); a daily cron (~06:00 UTC) sends a digest via `sendOverdueDigest()`, and `POST /api/push/test` sends it on demand. SW handles `push`/`notificationclick` ([public/sw.js](public/sw.js)). iOS requires the PWA installed (iOS 16.4+).
