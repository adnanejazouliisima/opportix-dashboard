      ...
      <header className="app-header" style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"10px 20px",background:"#fff",borderBottom:"1px solid #E5E5E3",flexWrap:"wrap",gap:8}}>
        ...
        <div style={{display:"flex",gap:6,alignItems:"center"}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:6}}>
            ...
            <button onClick={async()=>{try{const res=await fetch('/api/export/csv/monthly',{headers});if(!res.ok){setToast({msg:"Erreur export mensuel",type:"err"});return;}const blob=await res.blob();const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download=`opportix_mensuel_${new Date().toISOString().slice(0,10)}.csv`;a.click();URL.revokeObjectURL(url);setToast({msg:"Export mensuel téléchargé",type:"ok"});}catch{setToast({msg:"Erreur export mensuel",type:"err"});}}} style={{marginLeft:4,padding:"3px 10px",borderRadius:5,border:"1px solid #2FAA6B",background:"#fff",color:"#2FAA6B",fontSize:10,fontWeight:600,cursor:"pointer",fontFamily:"inherit"}}>Export mensuel</button>
