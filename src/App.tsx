...
function Dashboard({ session, onLogout }: any) {
  ...
  const sv = async (o: any = {}) => {
    ...
    const d = {
      urban: o.urban ?? urban, green: o.green ?? green, garage: o.garage ?? garage,
      deps: o.deps ?? deps, rets: o.rets ?? rets,
      disp: o.disp ?? disp, vacs: o.vacs ?? vacs, pros: o.pros ?? pros, msgs: o.msgs ?? msgs
    };
  ...
  const add = (type: string, entry: any) => {
    const m: any = { urban: [urban, setUrban, "urban"], green: [green, setGreen, "green"], garage: [garage, setGarage, "garage"], deps: [deps, setDeps, "deps"], rets: [rets, setRets, "rets"], disp: [disp, setDisp, "disp"], vacs: [vacs, setVacs, "vacs"], pros: [pros, setPros, "pros"] };
  ...
  const tog = (im: string) => {
    ...
    if (fTab === "urban") { const n = up(urban); setUrban(n); sv({ urban: n }); }
    else { const n = up(green); setGreen(n); sv({ green: n }); }
  };
  ...
  const toggleRole = async (id: number, r: string) => {
    const res = await fetch('/api/users/' + id + '/role', { method: 'PUT', headers, body: JSON.stringify({ role: r }) });
    if (res.ok) {
      setUsersList(usersList.map(u => u.id === id ? { ...u, role: r } : u));
      ...
      <header style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 20px", background: "#fff", borderBottom: "1px solid #E5E5E3", flexWrap: "wrap", gap: 8 }}>
        ...
        <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            <div style={{ width: 24, height: 24, borderRadius: "50%", background: "#1A1A1A", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10, fontWeight: 700, color: "#fff" }}>{user.displayName?.charAt(0)?.toUpperCase()}</div>
          ...
          <div className="ani">
            ...
            <div style={{ display: "flex", gap: 10, alignItems: "flex-start", flexWrap: "wrap" }}>
              <CrudP title="Comptes enregistrés" color="#1A1A1A" data={usersList} type="users" showAdd={showAdd} setShowAdd={setShowAdd}
                ...
                cols="30px 100px 150px 120px 100px 80px" heads={["", "IDENTIFIANT", "NOM", "RÔLE", "PÔLE", ""]}
                rr={(u: any) => <><span style={{ fontWeight: 800 }}>{u.role === 'admin' ? '🛡️' : '👤'}</span><span style={{ fontWeight: 600 }}>{u.username}</span><span>{u.displayName}</span><span>{u.role}{user.role === 'admin' && u.role !== 'admin' && <button onClick={() => toggleRole(u.id, u.role === 'editeur' ? 'lecteur' : 'editeur')} style={{ marginLeft: 6, fontSize: 9, padding: "1px 4px", borderRadius: 4, border: "1px solid #ddd", background: "#fff", cursor: "pointer" }}>Bascule</button>}</span><span>{u.pole}</span></>}
              />
            </div>
          </div>
        )}
...
function DiffBlock({ title, titleBg, color, count, heads, cols, data, maxH = 160, renderRow, formFields, onAdd, onDel, useIdx }: any) {
  const role = (() => { try { return JSON.parse(localStorage.getItem('net-session') || '{}')?.user?.role } catch { return '' } })();
  const [open, setOpen] = useState(false);
  const [f, setF] = useState<any>({});
  const [delId, setDelId] = useState<any>(null);
  const doAdd = () => { 
    const finalF = { ...f };
    formFields.forEach(([k, l, p, opts]: any) => { if (opts && !finalF[k]) finalF[k] = opts[0]; });
    onAdd(finalF); 
    setF({}); 
    setOpen(false); 
  };
  return (
    <div style={{ background: "#fff", borderRadius: 8, border: "1px solid #E5E5E3", overflow: "hidden" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "7px 12px", background: titleBg || "#F5F5F3", borderBottom: "1px solid #E5E5E3" }}>
        ...
        {role !== 'lecteur' && <button onClick={() => { setOpen(!open); setF({}); }} style={{ padding: "2px 8px", borderRadius: 4, border: `1px solid ${color}`, background: open ? color : "transparent", color: open ? "#fff" : color, fontSize: 10, fontWeight: 700, cursor: "pointer", fontFamily: "inherit", transition: "all .15s" }}>{open ? "Annuler" : "+"}</button>}
      </div>
      {open && (
...
function CrudP({ title, color, data, type, showAdd, setShowAdd, fields, form, setForm, addItem, delItem, cols, heads, rr, useIdx }: any) {
  ...
  return <div className="ani">
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
      ...
      {role !== 'lecteur' && <button onClick={() => { open ? setShowAdd(null) : setShowAdd(type); setForm({}); }} style={oBtn(color, open)}>{open ? "Annuler" : "+ Ajouter"}</button>}
    </div>
    {open && <div className="ani" style={{ background: "#fff", borderRadius: 8, padding: 14, border: "1px solid #E5E5E3", marginBottom: 10 }}>
      ...
      <button onClick={() => { 
        const req = type === "vacs" ? form.ch : type === "pros" ? form.nom : form.im; 
        if (!req?.trim()) return; 
        const finalForm = { ...form };
        fields.forEach(([l, k, p, opts]: any) => { if (opts && !finalForm[k]) finalForm[k] = opts[0]; });
        addItem(type, { ...finalForm, im: finalForm.im?.toUpperCase().trim() }); 
      }} style={{ marginTop: 10, padding: "7px 18px", borderRadius: 6, border: "none", background: "#1A1A1A", color: "#fff", fontSize: 11, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}>Ajouter</button>
    </div>}
    <div style={{ background: "#fff", borderRadius: 8, border: "1px solid #E5E5E3", overflow: "hidden" }}>
        ...
        {data.map((d: any, i: number) => <div key={useIdx ? i : d.id} className="rw" style={{ display: "grid", gridTemplateColumns: cols, padding: "7px 12px", borderBottom: "1px solid #F5F5F3", alignItems: "center", fontSize: 12 }}>
          ...
          <span style={{ textAlign: "right" }}>
            {!(type === 'users' && role !== 'admin') && (
              <button className="dl" onClick={() => delItem(type, useIdx ? i : d.id, useIdx)} style={{ padding: "2px 7px", borderRadius: 4, border: "1px solid #E8E8E5", background: "#fff", color: "#BBB", fontSize: 9, cursor: "pointer", fontFamily: "inherit" }}>Retirer</button>
            )}
          </span>
        </div>)}
